...
class PairCreatorService {
     ...
     */
    createFilePaths(targetDirectory, fileName, rule) {
        return {
            headerPath: vscode.Uri.file(path.join(targetDirectory.fsPath, `${fileName}${rule.headerExt}`)),
            sourcePath: vscode.Uri.file(path.join(targetDirectory.fsPath, `${fileName}${rule.sourceExt}`))
        };
    }
    ...
    static async fileExists(filePath) {
        ...
        const promise = Promise.resolve(vscode.workspace.fs.stat(vscode.Uri.file(filePath))
            .then(() => true, () => false));
    ...
    // decisions.
    async detectLanguageFromEditor() {
    ...
    // development)
    async detectLanguage(languageId, filePath) {
    ...
    async detectLanguageForHeaderFile(filePath) {
        ...
        const cppChecks = cppExtensions.map(ext => PairCreatorService.fileExists(path.join(dirPath, `${baseName}${ext}`)));
    ...
    getAllPairingRules() {
        return [
            ...(pairing_rule_manager_1.PairingRuleService.getRules('workspace') ?? []),
            ...(pairing_rule_manager_1.PairingRuleService.getRules('user') ?? [])
        ];
    }
    // Gets custom C++ extensions if available from configuration
    getCustomCppExtensions() {
        const allRules = this.getAllPairingRules();
        const cppCustomRule = allRules.find((rule) => rule.language === 'cpp');
    ...
    generateHeaderGuard(fileName, headerExt) {
    ...
    getDefaultPlaceholder(rule) {
        if (rule.isClass) {
            return templates_1.DEFAULT_PLACEHOLDERS.CPP_CLASS;
        }
        if (rule.isStruct) {
            return rule.language === 'cpp' ? templates_1.DEFAULT_PLACEHOLDERS.CPP_STRUCT
                : templates_1.DEFAULT_PLACEHOLDERS.C_STRUCT;
        }
        return rule.language === 'c' ? templates_1.DEFAULT_PLACEHOLDERS.C_EMPTY
            : templates_1.DEFAULT_PLACEHOLDERS.CPP_EMPTY;
    }
    // Optimized line ending detection based on VS Code settings and platform
    getLineEnding() {
        const eolSetting = vscode.workspace.getConfiguration('files').get('eol');
        return eolSetting === '\n' || eolSetting === '\r\n' ? eolSetting
    ...
    async checkFileExistence(headerPath, sourcePath) {
    ...
    async writeFiles(headerPath, sourcePath, headerContent, sourceContent) {
        try {
            await Promise.all([
    ...
    async getTargetDirectory(activeDocumentPath, workspaceFolders) {
    ...
    async shouldShowLanguageMismatchWarning(language, result, currentDir, activeFilePath) {
    ...
    async checkForCppFilesInDirectory(dirPath) {
        try {
            const entries = await vscode.workspace.fs.readDirectory(vscode.Uri.file(dirPath));
            const hasCppFiles = entries.some(([fileName, fileType]) => fileType === vscode.FileType.File &&
    ...
    async checkForCorrespondingSourceFiles(dirPath, filePath, language) {
        const baseName = path.basename(filePath, path.extname(filePath));
        const extensions = language === 'c' ? ['.c'] : ['.cpp', '.cc', '.cxx'];
        const checks = extensions.map(ext => PairCreatorService.fileExists(path.join(dirPath, `${baseName}${ext}`)));
        try {
            const results = await Promise.all(checks);
     ...
     */
    async handleOfferToSaveAsDefault(rule, language) {
        ...
        const choice = await vscode.window.showInformationMessage(`Files created successfully! Would you like to save "${rule.headerExt}/${rule.sourceExt}" as your default C++ extensions for this workspace?`, 'Save as Default', 'Not Now');
    ...
    async saveRuleAsDefault(rule) {
        const { PairingRuleService } = await Promise.resolve().then(() => __importStar(require('../pairing-rule-manager')));
        ...
        try {
            ...
            await PairingRuleService.writeRules([cleanRule], scopeChoice.scope);
    ...
    async generateAndWriteFiles(fileName, rule, headerPath, sourcePath) {
