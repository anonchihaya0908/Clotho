...
export class PairingRuleService {
    ...
    getActiveRules(): ReadonlyArray<PairingRule> {
        return vscode.workspace.getConfiguration('clotho').get<PairingRule[]>(
            CONFIG_KEYS.CREATE_PAIR_RULES, []);
    }
    ...
    hasCustomRules(scope: 'workspace' | 'user'): boolean {
        const inspection =
            vscode.workspace.getConfiguration('clotho').inspect<PairingRule[]>(
                CONFIG_KEYS.CREATE_PAIR_RULES);
     ...
     */
    async writeRules(rules: PairingRule[], scope: 'workspace' | 'user'): Promise<void> {
        const saveRules = ErrorHandler.wrapAsync(async () => {
            ...
            await vscode.workspace.getConfiguration('clotho').update(
                CONFIG_KEYS.CREATE_PAIR_RULES, rules, target);
        }, {
    ...
    async resetRules(scope: 'workspace' | 'user'): Promise<void> {
        const resetRules = ErrorHandler.wrapAsync(async () => {
            const target = scope === 'workspace'
                ? vscode.ConfigurationTarget.Workspace
                : vscode.ConfigurationTarget.Global;
            await vscode.workspace.getConfiguration('clotho').update(
                CONFIG_KEYS.CREATE_PAIR_RULES, undefined, target);
        }, {
