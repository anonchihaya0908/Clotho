...
interface RecoveryStrategy {
    recover(error: EditorError, stateManager: EditorStateManager, eventBus: EventBus): Promise<void>;
}
...
export class ErrorRecoveryManager implements vscode.Disposable {
    ...
    async handleError(errorCode: string, error: any, context?: Record<string, any>): Promise<void> {
        ...
        console.error(`[ErrorRecovery] Code: ${errorCode}. Message: ${editorError.message}`, { error: editorError });
    ...
    private async handleUnrecoverableError(error: EditorError): Promise<void> {
        console.error(`Unrecoverable error occurred: ${error.code}. Resetting to safe state.`);
        await this.stateManager.resetToSafeState();
        vscode.window.showErrorMessage(`Clang-Format Editor遇到严重错误，已重置到安全状态。详情请查看日志。`);
    }
    ...
    private async attemptRecovery(errorCode: string, error: EditorError): Promise<void> {
        ...
        if (currentState.recoveryAttempts >= this.maxRecoveryAttempts) {
            console.error(`Maximum recovery attempts reached for error: ${errorCode}. Resetting to safe state.`);
            await this.stateManager.resetToSafeState();
            vscode.window.showWarningMessage(`插件多次尝试恢复失败，已重置到安全状态。`);
            return;
        }

        console.log(`Attempting recovery for ${errorCode} (Attempt ${currentState.recoveryAttempts + 1})`);
    ...
    private setupRecoveryStrategies(): void {
        ...
        this.recoveryStrategies.set('preview-creation-failed', {
            async recover(error, stateManager, eventBus) {
                console.error('Recovery: Preview creation failed and no fallback is available.', error);
                await stateManager.updateState({ previewMode: 'closed' }, 'preview-creation-failed');
        ...
        this.recoveryStrategies.set('message-handling-failed', {
            async recover(error, stateManager) {
                ...
                // 清除错误状态，因为这是一个非关键错误
                await stateManager.updateState({ lastError: undefined }, 'ignore-message-error');
            }
        });
    }
