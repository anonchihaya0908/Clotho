 ...
 * ClangFormat面板管理器实现
 */
export class ClangFormatPanelManager implements PanelManager, vscode.Disposable {
    ...
    createPanel(id: string, options: PanelOptions): vscode.WebviewPanel {
        try {
            ...
            if (existingPanel && !existingPanel.disposed) {
                console.log(`PanelManager: Panel ${id} already exists, revealing existing panel`);
     ...
     * 设置面板事件监听器
     */
    private setupPanelEventListeners(id: string, panel: vscode.WebviewPanel): void {
        // 监听面板可见性变化
