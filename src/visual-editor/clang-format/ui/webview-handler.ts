...
import { WebviewMessage, WebviewMessageType } from '../../../common/types/index';
...
export class ClangFormatWebviewHandler implements WebviewMessageHandler {
    ...
    async handleMessage(message: WebviewMessage): Promise<void> {
        try {
            console.log(`WebviewHandler[${this.instanceId}]: Received message ${message.type}`);
            ...
            switch (message.type) {
                ...
                default:
                    console.warn(`WebviewHandler[${this.instanceId}]: Unknown message type: ${message.type}`);
            }
    ...
    private async handleConfigChange(payload: any): Promise<void> {
        ...
        console.log(`WebviewHandler[${this.instanceId}]: Config updated - ${key}: ${value}`);
    }
    ...
    private async handleExportConfig(): Promise<void> {
        try {
            ...
            if (saveUri) {
                const configContent = this.formatService.generateConfigFile(this.currentConfig);
    ...
    private async handleResetConfig(): Promise<void> {
        ...
        if (choice === 'Reset') {
            ...
            console.log(`WebviewHandler[${this.instanceId}]: Configuration reset to defaults`);
        }
    }
    ...
    private async handleValidateConfig(): Promise<void> {
        ...
        await this.sendMessage({
            type: validation.isValid ? WebviewMessageType.VALIDATION_RESULT : WebviewMessageType.VALIDATION_ERROR,
            payload: validation
        });
    }
    ...
    private async handleUpdateSettings(payload: any): Promise<void> {
        try {
            ...
            await config.update('showGuideButton', showGuideButton, vscode.ConfigurationTarget.Workspace);
    ...
    private async handleConfigOptionHover(payload: { optionName: string }): Promise<void> {
        // 这里可以实现hover时的高亮逻辑
        console.log(`WebviewHandler[${this.instanceId}]: Option hover - ${payload.optionName}`);
    }
    ...
    private async handleConfigOptionFocus(payload: { optionName: string }): Promise<void> {
        // 这里可以实现focus时的滚动逻辑
        console.log(`WebviewHandler[${this.instanceId}]: Option focus - ${payload.optionName}`);
    }
