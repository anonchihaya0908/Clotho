...
export class MultiInstanceClangFormatCoordinator implements vscode.Disposable {
    ...
    async showEditor(source: EditorOpenSource = EditorOpenSource.DIRECT): Promise<void> {
        try {
            ...
            if (this.instanceManager.focusWorkspaceEditor(workspaceFolder)) {
                console.log('MultiInstanceCoordinator: Focused existing editor instance');
                return;
            }
            ...
            console.log(`MultiInstanceCoordinator: Created new editor instance for ${workspaceFolder?.name || 'default'}`);
    ...
    async showNewEditor(): Promise<void> {
        try {
            ...
            console.log('MultiInstanceCoordinator: Created new editor instance (forced)');
    ...
    private registerCommands(): void {
        ...
        // 强制创建新实例的命令
        this.disposables.push(
            vscode.commands.registerCommand(COMMANDS.OPEN_CLANG_FORMAT_EDITOR + '.new', async () => {
        ...
        // 关闭所有实例的命令
        this.disposables.push(
            vscode.commands.registerCommand(COMMANDS.OPEN_CLANG_FORMAT_EDITOR + '.closeAll', async () => {
        ...
        // 显示统计信息的命令
        this.disposables.push(
            vscode.commands.registerCommand(COMMANDS.OPEN_CLANG_FORMAT_EDITOR + '.stats', () => {
