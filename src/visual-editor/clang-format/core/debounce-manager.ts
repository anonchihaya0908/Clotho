...
export class DebounceManager {
        ...
        try {
            ...
            if (this.locks.has(key)) {
                if (options.throwOnTimeout !== false) {
                    // 使用更友好的日志级别
                    console.log(`DebounceManager: Operation ${key} is already in progress, skipping duplicate call`);
     ...
     * 队列化操作
     */
    async queueOperation(key: string, operation: () => Promise<void>): Promise<void> {
