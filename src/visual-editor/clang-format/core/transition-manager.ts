...
export class TransitionManager {
    ...
    async switchToEasterEgg(
        ...
        const switchOperation = this.debounceManager.debounce(
            ...
            async () => {
                return await this.debounceManager.withLock('webview-transition', async () => {
    ...
    private async performEasterEggTransition(
        ...
        try {
            ...
            console.log(`TransitionManager: Easter egg transition completed in ${transitionTime}ms`);
    ...
    private async performPreviewTransition(
        ...
        try {
            ...
            const transitionTime = Date.now() - this.transitionStartTime;
            console.log(`TransitionManager: Preview transition completed in ${transitionTime}ms`);
    ...
    private async loadEasterEggContent(
        ...
        if (this.options.contentLoadDelay! > 0) {
            await new Promise(resolve => setTimeout(resolve, this.options.contentLoadDelay));
        }
     ...
     * 用实际内容替换占位符
     */
    private async replacePlaceholderWithContent(contentPanel: vscode.WebviewPanel): Promise<void> {
        try {
            // 确保内容面板在正确的位置
    ...
    getStats(): {
        ...
        return {
            ...
            elapsedTime: this.transitionStartTime > 0 ? Date.now() - this.transitionStartTime : 0,
