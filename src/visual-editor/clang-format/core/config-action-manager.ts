...
export class ConfigActionManager implements BaseManager {
    ...
    private setupEventListeners(): void {
        ...
        // 监听UI的动作请求
        eventBus.on('load-workspace-config-requested', () => this.handleLoadWorkspaceConfig());
        ...
        eventBus.on('open-clang-format-file-requested', () => this.handleOpenClangFormatFile());
    ...
    private async loadConfigFromFile(fileUri: vscode.Uri): Promise<void> {
        try {
            ...
            await this.context.errorRecovery.handleError('config-load-failed', error, { file: fileUri.toString() });
    ...
    private async writeConfigToFile(fileUri: vscode.Uri): Promise<void> {
        try {
            ...
            await this.context.stateManager.updateState({ configDirty: false }, 'config-saved');
            vscode.window.showInformationMessage(`Configuration saved to ${vscode.workspace.asRelativePath(fileUri)}.`);
        } catch (error: any) {
            await this.context.errorRecovery.handleError('config-save-failed', error, { file: fileUri.toString() });
