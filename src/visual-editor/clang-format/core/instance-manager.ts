...
export class ClangFormatInstanceManager extends BaseInstanceManager<ClangFormatEditorInstance> {
  ...
  ): ClangFormatEditorInstance {
    const id = this.generateInstanceId(workspaceFolder);

    return this.create(id, () => {
      const instance = new ClangFormatEditorInstanceImpl(
        id,
        this.extensionUri,
        workspaceFolder,
      );
  ...
  updateInstanceState(
    id: string,
    updates: Partial<ClangFormatInstanceState>,
  ): void {
  ...
  protected performCleanup(): void {
    ...
    for (const [id, state] of this.instanceStates) {
      ...
      if (
        ...
        console.log(
          `ClangFormatInstanceManager: Cleaning up inactive instance ${id}`,
        );
        this.destroy(id);
      }
    }
...
class ClangFormatEditorInstanceImpl implements ClangFormatEditorInstance {
  ...
  async handleMessage(message: any): Promise<void> {
    try {
      // 基础消息处理逻辑
      console.log(
        `ClangFormatEditorInstance: Received message ${message.type} for instance ${this.id}`,
      );
