...
export class StatusMonitor implements IMonitor {
        ...
        try {
            ...
            try {
                ...
                if (version) {
                    ...
                    console.debug('Clotho StatusMonitor: Found backend clangd version:', version);
                } else {
                    status.version = '后端版本检测失败';
                    console.debug('Clotho StatusMonitor: Failed to detect backend clangd version');
                }
            } catch (error) {
                status.version = '后端版本检测错误';
                console.debug('Clotho StatusMonitor: Error detecting backend version:', error);
            }
    ...
    private async getClangdBackendVersion(): Promise<string | null> {
        ...
        console.debug('Clotho StatusMonitor: Both version detection methods failed');
        return null;
    }
     ...
     */
    private async tryGetVersionFromPath(clangdPath: string): Promise<string | null> {
        return new Promise((resolve) => {
            ...
            exec(command, { timeout: 5000 }, (error, stdout, stderr) => {
                if (error) {
                    console.debug(`Clotho StatusMonitor: ${clangdPath} --version failed:`, error.message);
                    resolve(null);
                    return;
                }
                ...
                if (version) {
                    console.debug(`Clotho StatusMonitor: Version from ${clangdPath}:`, version);
                }
                resolve(version);
            });
        });
    ...
    private async tryGetVersionFromVSCodeSettings(): Promise<string | null> {
        try {
            ...
            console.debug('Clotho StatusMonitor: VS Code clangd.path setting:', clangdPath);
            ...
            console.debug('Clotho StatusMonitor: Error reading VS Code settings:', error);
            return null;
        }
    }
    ...
    private parseVersionFromOutput(output: string): string | null {
        ...
        console.debug('Clotho StatusMonitor: Parsing version from output:', trimmedOutput);
