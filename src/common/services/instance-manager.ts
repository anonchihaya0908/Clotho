 ...
 * 通用实例管理器实现
 */
export class BaseInstanceManager<T extends Disposable> implements InstanceManager<T>, vscode.Disposable {
    ...
    create(id: string, factory: () => T): T {
        try {
            ...
            if (this.instances.has(id)) {
                ...
                console.log(`InstanceManager: Instance ${id} already exists, returning existing instance`);
                return existing;
            }

            // 检查实例数量限制
            if (this.config.maxInstances && this.instances.size >= this.config.maxInstances) {
    ...
    destroy(id: string): boolean {
        try {
            ...
            console.log(`InstanceManager: Destroyed instance ${id}, remaining: ${this.instances.size}`);
            return true;
    ...
    protected performCleanup(): void {
        // 基础实现：检查实例是否仍然有效
        // 子类可以重写此方法添加更复杂的清理逻辑
        console.log(`InstanceManager: Performing cleanup, current instances: ${this.instances.size}`);
    }
