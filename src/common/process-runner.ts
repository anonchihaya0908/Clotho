...
export class ProcessRunner {
    private static readonly DEFAULT_OPTIONS: Required<Omit<CommandOptions, 'cwd'>> = {
     ...
     */
    public static async runCommand(command: string, options: CommandOptions = {}): Promise<string> {
     ...
     */
    public static async runCommandWithDetails(command: string, options: CommandOptions = {}): Promise<CommandResult> {
        const mergedOptions = { ...ProcessRunner.DEFAULT_OPTIONS, ...options };

        if (mergedOptions.logCommand) {
            console.debug(`ProcessRunner: Executing command with details: ${command}`);
        }
     ...
     */
    public static async getProcessInfo(processName: string): Promise<Array<{ pid: number, ppid: number, memory: number }>> {
        try {
            ...
            console.debug(`ProcessRunner: Failed to get process info for ${processName}:`, error);
            return [];
        }
    }
     ...
     */
    private static async getWindowsProcessInfo(processName: string): Promise<Array<{ pid: number, ppid: number, memory: number }>> {
        ...
        try {
            ...
            const stdout = await ProcessRunner.runCommand(wmicCommand, { logCommand: true });
            ...
            console.debug('ProcessRunner: WMIC failed, trying PowerShell fallback:', wmicError);
            ...
            const psOutput = await ProcessRunner.runCommand(psCommand, { logCommand: true });
     ...
     */
    private static async getUnixProcessInfo(processName: string): Promise<Array<{ pid: number, ppid: number, memory: number }>> {
        const command = `ps -eo pid,ppid,rss,comm | grep ${processName} | grep -v grep`;
        const stdout = await ProcessRunner.runCommand(command, { logCommand: true });
