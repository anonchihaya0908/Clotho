...
export class ProcessDetector {
     ...
     */
    public static async findMainProcessByName(processName: string): Promise<ProcessInfo | undefined> {
        try {
            console.log(`🕵️ ProcessDetector: Starting investigation for "${processName}" processes...`);
            ...
            console.log(`📊 Found ${allTargetProcesses.length} ${processName} process(es) in the system:`);
            allTargetProcesses.forEach(p => {
                const memMB = Math.round(p.memory / 1024);
                console.log(`   Process PID ${p.pid}: Parent=${p.ppid}, Memory=${memMB}MB`);
            });

            // Step 3: Perform the "DNA Test" - identify our legitimate children
            const result = ProcessDetector.performDnaTest(ourPid, allTargetProcesses, processName);

            if (result) {
                console.log(`✅ ProcessDetector: Successfully identified main ${processName} process!`);
                console.log(`   🎯 Selected PID ${result.pid}: ${Math.round(result.memory / 1024)}MB (${result.relationship})`);
            } else {
                console.log(`❌ ProcessDetector: Failed to identify main ${processName} process`);
            }

            return result;

        } catch (error) {
            console.error(`ProcessDetector: Failed to find main process for ${processName}:`, error);
    ...
    private static performDnaTest(
        ...
        const potentialGrandchildren = allProcesses.filter(p => p.ppid !== ourPid);

        console.log('🔬 DNA Analysis Results:');
        console.log(`   Direct children (PPID=${ourPid}): ${directChildren.length}`);
        console.log(`   Potential grandchildren/orphans: ${potentialGrandchildren.length}`);
        ...
        if (potentialGrandchildren.length > 0) {
            ...
            potentialGrandchildren.forEach(p => {
                const memMB = Math.round(p.memory / 1024);
                console.log(`   ⚠️ PID ${p.pid}: Parent=${p.ppid}, ${memMB}MB (Unknown relationship)`);
            });
        }
        ...
        if (candidates.length === 0) {
            console.log('⚠️ No direct children found, considering potential grandchildren...');
        ...
        console.log(`   🎯 Selected PID ${selectedProcess.pid}: ${memMB}MB (${selectedRelationship})`);
    ...
    public static async detectProcessWithStrategy(
        ...
        try {
            ...
            if (apiDetector) {
                console.log(`🎯 Strategy 1: Attempting API detection for ${processName}...`);
            ...
            const mainProcess = ProcessDetector.performDnaTest(process.pid, allProcesses, processName);
            ...
            console.error(`ProcessDetector: Advanced detection failed for ${processName}:`, error);
            return {
                success: false,
                method: 'failed',
                candidateCount: 0,
    ...
    public static async getDiagnosticInfo(processName: string): Promise<{
        ...
        const directChildren = processInfos.filter(p => p.relationship === 'direct-child');
        const orphans = processInfos.filter(p => p.relationship === 'orphan');

        const recommendations: string[] = [];

        if (directChildren.length === 0 && orphans.length > 0) {
            recommendations.push('No direct children found - orphan processes may be stale from previous sessions');
            recommendations.push('Consider restarting VS Code to clean up stale processes');
        }

        if (directChildren.length > 1) {
            recommendations.push('Multiple direct children found - this is normal for clangd');
            recommendations.push('The process with highest memory usage will be selected as main');
        }
